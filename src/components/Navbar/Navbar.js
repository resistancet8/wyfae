...
class NavbarComponent extends Component {
	...
	componentDidMount() {
		if (this.props.auth.isAuthenticated) this.getNotifications();
	...
	loadFunc() {
		Axios.post(`${process.env.REACT_APP_API_ENDPOINT}/user/get_notification`, {
			skip_count: this.state.notifications.length,
			limit_count: 10
		})
			.then((response) => {
				this.setState((prev) => {
	...
	render() {
		...
		let Results =
			this.state.results && this.state.results.length > 0 ? (
				this.state.results.map((o) => {
					return <SearchItem user={o} handleUrlChange={this.handleUrlChange.bind(this)} />;
				})
			) : (
				<SearchItem empty={1} />
			);
					...
					notificationHTML.push(
						...
						>
							...
							>
								...
								>
									<strong className="font-weight-bold">{notification.name}</strong>
								</Link>{' '}
								liked <strong className="font-weight-bold">{notification.post_title}</strong>.{' '}
								<div class="pl-2 mt-2">
									<strong>{moment(notification.creation_time).local().fromNow()}</strong>
								</div>
							</ListGroupItem>
						</Link>
					);
					...
					notificationHTML.push(
						...
						>
							...
							>
								...
								unliked <strong className="font-weight-bold">{notification.post_title}</strong>.{' '}
								<div class="pl-2 mt-2">
									<strong>{moment(notification.creation_time).local().fromNow()}</strong>
								</div>
							</ListGroupItem>
					...
					notificationHTML.push(
						...
						>
							...
							>
								...
								commented <strong className="font-weight-bold">{notification.comment_text}</strong> on{' '}
								<strong className="font-weight-bold">{notification.post_title}</strong>.{' '}
								<div class="pl-2 mt-2">
									<strong>{moment(notification.creation_time).local().fromNow()}</strong>
								</div>
							</ListGroupItem>
						...
						>
							...
							>
								<i class="fas fa-user-plus" />&nbsp;&nbsp;&nbsp;<Link
									to={`/profile/${notification.followed_by}`}
								>
									<strong className="font-weight-bold">{notification.followed_by}</strong>
								</Link>{' '}
		...
		return (
			<header>
				<div
					className={`d-flex flex-column flex-md-row align-items-center p-3 px-md-4 bg-white border-bottom shadow-sm navbar-holder-design justify-content-between ${window.location.pathname.indexOf(
						'login'
					) >= 0 || window.location.pathname.indexOf('register') >= 0
						? ''
						: 'add-gradient'} `}
				>
